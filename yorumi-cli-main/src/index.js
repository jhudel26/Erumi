...
const main = async () => {
    ...
    if (options.latest) {
        ...
        if (!isSpecialQuery) {
            results.sort((a, b) => {
                ...
                if (aTitle === qLower && bTitle !== qLower)
                    return -1;
                if (bTitle === qLower && aTitle !== qLower)
                    return 1;
                return 0;
            });
        }
    ...
    let anime = requestedAnimeIndex > 0 && requestedAnimeIndex <= visibleResults.length
        ...
        : await chooseFromList('Anime', visibleResults, (item) => `${item.title}${item.year ? ` (${item.year})` : ''}${item.episodes ? ` - ${item.episodes} eps` : ''}`);
    if (!options.json)

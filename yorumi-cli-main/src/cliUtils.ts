...
export const fmtLabel = (tag: string, bg: string, msg: string) =>
  `  ${CLR.black}${bg} ${tag} ${CLR.reset}  ${msg}`;

const sleep = (ms: number) => new Promise<void>((r) => setTimeout(r, ms));

export const drawBar = (filled: number, text: string) => {
...
export const msgAbove = (filled: number, barText: string, msg: string) => {
  process.stdout.write(`\r${ERASE_LINE}`);
  console.log(msg);
  drawBar(filled, barText);
};

export const animateBar = async (fromFilled: number, targetStep: number, totalSteps: number, text: string) => {
